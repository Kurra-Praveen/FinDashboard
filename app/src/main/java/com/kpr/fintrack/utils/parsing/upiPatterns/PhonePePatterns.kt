...
object PhonePePatterns {
    ...
    private  val debitPattern = listOf(
        ParsingPatternRepository.TransactionPattern(
            ...
            regex = Pattern.compile(
                "Transaction Successful\\s+((\\d{2}:\\d{2}\\s+[ap]m)\\s+on\\s+(\\d{2}\\s+\\w+\\s+\\d{4}))\\s+Received from\\s+(.+?)\\s+(\\+91\\d{10})\\s+Banking Name\\s*:\\s(.+?)\\s+Transfer Details\\s+Transaction ID\\s+([A-Z0-9]+)\\s+Credited to\\s+([Xx*]+[\\d]+)\\s+UTR:\\s+(\\d+)\\s+.*?([\\d.,]+)\\s+([\\d.,]+)",
    ...
    fun getPhonePePatterns(): List<ParsingPatternRepository.TransactionPattern> {
        return listOf(PhonePePatterns.debitPattern).flatten()
    }
}